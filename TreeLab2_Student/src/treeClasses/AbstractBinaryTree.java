...
public abstract class AbstractBinaryTree<E> extends AbstractTree<E> implements
	...
	protected void recDisplay(Position<E> root, int[] control, int level) 
	{
		...
		printPrefix(level, control);                                     //
		if(this.isRoot(root))
		System.out.println("__(ROOT)("+root.getElement()+")"); 
		//If the element position I have is the left of its Parent do this
		else if(left(parent(root))==root){
			System.out.println("__L("+root.getElement()+")"); 
		}
		//If the element position I have is the left of its Parent do this
		else if(right(parent(root))==root){
